    ...
    const gen = paginate<Partial<Deal>>(
      ...
      async (input) => {
        ...
        $('div[data-testid="marketplace-search-item"]').each((_: number, el: any) => {
          const row = $(el)
          const title = row.find('span[data-testid="marketplace-search-item-title"]').text().trim()
          if (!title) return

          const priceText = row.find('span[data-testid="marketplace-search-item-price"]').text().trim()
          const price = extractPrice(priceText)
          if (!price) return

          const locationText = row.find('span[data-testid="marketplace-search-item-location"]').text().trim()
          const descriptionText = row.find('div[data-testid="marketplace-search-item-description"]').text().trim()
          const link = row.find('a[href*="/marketplace/item/"]').attr('href')
          const imgSrc = row.find('img[data-testid="marketplace-search-item-image"]').attr('src')
          const itemId = link?.split('/marketplace/item/')[1]?.split('/')[0] || ''
        ...
        // Facebook uses infinite scroll; pagination is not reliable
        const hasMore = items.length > 0 && config.maxPages > 1
        return { items, hasMore: hasMore && (Math.random() > 0.3) }
      }
    )
