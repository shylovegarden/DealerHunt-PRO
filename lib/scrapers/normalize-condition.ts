...
export function normalizeCondition(
  ...
  const x = String(raw).toLowerCase().trim().replace(/[_\s-]+/g, " ");
  ...
  if (/\b(non[\s-]?repairable|parts only|parts car|cert(ificate)? of destruction|scrap|junk|crush|dismantle)\b/.test(x))
    return "parts_only";
